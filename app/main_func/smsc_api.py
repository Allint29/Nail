...
try:
    from urlib import urlopen, quote
except ImportError:
    from urllib.request import urlopen
    from urllib.parse import quote

# Константы для настройки библиотеки
#SMSC_LOGIN = current_app.config['MY_SMSC_LOGIN']	    # логин клиента
#SMSC_PASSWORD = current_app.config['MY_SMSC_PASSWORD']	# пароль
...
class SMSC(object):
    ...
    def send_sms_mail(self, phones, message, translit=0, time="", id=0, format=0, sender=""):
        ...
        if current_app.config['SMTP_LOGIN']:
            server.login(current_app.config['SMTP_LOGIN'], current_app.config['SMTP_PASSWORD']) 

        server.sendmail(current_app.config['SMTP_FROM'], "send@send.smsc.ru", "Content-Type: text/plain; charset=" + \
            current_app.config['SMSC_CHARSET'] + "\n\n" + current_app.config['SMSC_LOGIN'] + ":" + current_app.config['SMSC_PASSWORD'] + ":" + str(id) + ":" + time + \
            ":" + str(translit) + "," + str(format) + "," + sender + ":" + phones + ":" + message)
        server.quit()
    ...
    def _smsc_send_cmd(self, cmd, arg=""):
        ...
        _url = url
        arg = "login=" + quote(current_app.config['SMSC_LOGIN']) + "&psw=" + quote(current_app.config['SMSC_PASSWORD']) + "&fmt=1&charset=" + current_app.config['SMSC_CHARSET'] + "&" + arg
