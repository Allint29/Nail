...
from app import db
from app.user.main.forms import EditProfileForm, PostForm, SearchForm
...
def index():
    ...
    helo="Вы на главной странице своего профиля."
    form = PostForm()
    ...
    return render_template('user/index.html', title=titleVar, form=form, posts=posts.items, next_url=next_url, helllo=helo,
                           prev_url=prev_url)


@bp.route('/user/explore')
@login_required
def explore():
    titleVar=_('Обзор')
    ...
    prev_url = url_for('main.explore', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('user/index.html', title=titleVar, posts=posts.items, next_url=next_url, prev_url=prev_url, helllo=helo)
...
def edit_profile():
    ...
    return render_template('user/edit_profile.html', title=_('Редактирование профиля'), form=form)
