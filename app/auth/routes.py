...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(_('Поздравляем, Вы зарегистрированы в системе!'))
        return redirect(url_for('auth.login'))
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash(_('Перейдите по ссылке на почте, для смены пароля.'))
        return redirect(url_for('auth.login'))
